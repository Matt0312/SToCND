...
class User():
    ...
    def get_seasonal(self, t):
    
        No_days = int((t) % workweek)
        time_of_day = (t  % 1) * secsinday
        if time_of_day < self.dailycutoffs[0] or time_of_day > self.dailycutoffs[-1]:
            Total = self.weekly_multipliers[No_days] * self.dailyrates[-1]
            
            return Total
        else:
            for cps in range(1,len(self.dailycutoffs)):
                if time_of_day < self.dailycutoffs[cps]:
                    Total = self.weekly_multipliers[No_days] * self.dailyrates[cps-1]
...
################ FUNCTIONS TO GET PVALUES
    ...
    #def get_pvalue_HS(self,waiting_time):
...
def C2(list, censored):  #######FIX TO JUST BE LOG#
    m = mean(list)
    if m == 0:
        m = 1
...
def C(list,index):  #######FIX TO JUST BE LOG#
    m = mean(list)
    if m == 0:
        m = 1
    likelihood=expon.pdf(list,0,m)
