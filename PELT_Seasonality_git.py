...
def step_function_PELT(times2):
    while len(times2)>100:
        times3 = sessionize2(times2)[1]
        times2 = times3
        print (len(times3),'length of sessionized times')
    beta =  2 * log(len(times2))
    ...
    #print ([Wrapped[i] for i in cps], rates, cps)
